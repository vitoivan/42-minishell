...
AstNode * expression_builder_semicolon(ParserContext *context, AstNode *node, AstNode *tmp) {
	...
	if (node != NULL && node->type == NODE_SEMICOLON) {
		...
		operator_node = mk_node_binary_expression(get_current_token(context), NULL, NULL);
		operator_node->as.expressions.nodes = lkd_lst_new_list();
		lkd_lst_add_back(&operator_node->as.expressions.nodes, lkd_lst_new_node(tmp));
	}
